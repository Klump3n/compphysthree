...
int check_device_host_sweep()
{
   double *h_rnd, *d_rnd, h_m, d_m, h_acc, d_acc, d, delta;
   ...
   h_rnd=randgpu(nvol*3*NTRIAL);
   d_rnd=devRandgpu(0);

   // sweep Host
   printf("Update Host\n");
   h_m=cuCabs(magnet());
   printf(" Magnetisierung vor Update: %f\n",h_m);
   h_acc=metro_sweep(delta,h_rnd,0);
   h_m=cuCabs(magnet());
   printf(" Magnetisierung nach Update: %f\n",h_m);
   printf(" Akzeptanz: %f\n",h_acc);
   ...
   d_m=cuCabs(magnet());
   printf(" Magnetisierung vor Update: %f\n",d_m);
   d_acc=metro_sweep(delta,d_rnd,1);
   d_m=cuCabs(magnet());
   printf(" Magnetisierung nach Update: %f\n",d_m);
   printf(" Akzeptanz: %f\n",d_acc);
...
int main(int argc, char **argv)
{
   ...
   printf("Sweep Device und Host:\n");
   if (check_device_host_sweep())
      printf(" Check5 erfolgreich.\n\n");
   else
      printf(" Check5 fehlgeschalgen!.\n\n");
